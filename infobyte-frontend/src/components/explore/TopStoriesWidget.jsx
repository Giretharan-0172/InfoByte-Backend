...
export default function TopStoriesWidget({ articles, onArticleClick }) {
  return (
    <div className="bg-navy-800/75 backdrop-blur-lg border border-gray-700/50 rounded-2xl h-full flex flex-col">
      ...
      {/* Limit visible topics to ~7. Scroll for more. Hide scrollbar. */}
      <div className="flex-1 overflow-y-auto scrollbar-hide max-h-96 p-4 space-y-4">
        {articles.length > 0 ? articles.map(article => (
          ...
          >
            <span className="text-xs text-gray-400">
              {article.category} - {formatTimeAgo(article.createdAt)}
            </span>
